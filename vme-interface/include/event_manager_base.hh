...
public:
  ...
  virtual int ResizeEventData(event_data_t &data) = 0;

  // Returns the oldest stored event.
  inline const event_data_t &GetCurrentEvent() { return data_queue_.front(); };
...
protected:
  ...
  std::queue<event_data_t> data_queue_;
  WfdContainer workers_;
