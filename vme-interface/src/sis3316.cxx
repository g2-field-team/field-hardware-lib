...
void Sis3316::LoadConfig()
{
  ...
  for (int gr = 0; gr < kNumChannelGroups; ++gr) {
  ...
  for (gr = 0; gr < kNumChannelGroups; ++gr) {
    ...
    msg = (trace_len_ << 16) | (0 & 0xffff); // 0 is start address in ADC
  ...
  for (int gr = 0; gr < kNumChannelGroups; ++gr) {
    ...
    read_len_ = 1 * (3 + trace_len_ / 2);
    rc = Write(addr, read_len_ - 1);
...
void Sis3316::GetEvent(wfd_data_t &bundle)
{
  ...
  for (ch = 0; ch < num_ch_; ch++) {
    ...
    offset += 0x4 * (ch % kNumChannelGroups);
    ...
    bundle.dev_clock[ch] = data[1] & 0xffff;
    bundle.dev_clock[ch] |= data[1] & (0xffff << 16);
    bundle.dev_clock[ch] |= (data[0] & 0xffffULL << 16) << 32;

    // Resize the data vector and copy the trace.
    bundle.trace[ch].resize(trace_len_);

    std::copy((ushort *)(&data[0] + 3),
	      (ushort *)(&data[0] + 3) + trace_len_,
    	      (ushort *)&bundle.trace[ch][0]);
  }
...
int Sis3316::I2cRead(int osc, unsigned char &data, unsigned char ack)
{
  if (osc < 0 || osc > 3) {
    LogError("I2C oscillator out of range, 0-3");
    return 1;
...
int Sis3316::I2cWrite(int osc, unsigned char data, unsigned char &ack)
{
  if (osc < 0 || osc > 3) {
    LogError("I2C oscillator out of range, 0-3");
    return 1;
...
int Sis3316::SetOscFreqHSN1(int osc, unsigned char hs, unsigned char n1)
{
...
int Sis3316::AdcSpiWrite(int gr, int chip, uint spi_addr, uint msg)
{
  int rc;
...
int Sis3316::AdcSpiRead(int gr, int chip, uint spi_addr, uint &msg)
{
  int rc;
  uint data = 0, adc_mux = 0, addr = 0, count = 0, maxcount = 1000;
...
int Sis3316::Si5325Read(uint addr, uint &msg)
{
...
int Sis3316::Si5325Write(uint addr, uint msg)
{
  uint count = 0, maxpoll = 100, tmp = 0;
